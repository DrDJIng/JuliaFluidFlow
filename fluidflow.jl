...
function vel_step(N, u, u0, v, v0, visc, dt)
    ...
    u[:, :], v[:, :] = project(N, u, v, zeros(xSize, ySize), zeros(xSize, ySize))
    u0[:, :], u[:, :] = swap(u0, u)
    v0[:, :], v[:, :] = swap(v0, v)
...
anim = @animate for tc = 1:tsteps
    ...
    u[:, :], v[:, :] = vel_step(N, u, u_prev, v, v_prev, visc, dt)
    dens[:, :] = dens_step(N, dens, dens_prev, u, v, diff, dt)
    p1 = heatmap(1:xSize, 1:ySize, dens, clim = (0, 4), aspect_ratio=:equal)
    p2 = heatmap(1:xSize, 1:ySize, u, aspect_ratio=:equal)
    p3 = heatmap(1:xSize, 1:ySize, v, aspect_ratio=:equal)
